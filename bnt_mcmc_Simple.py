...
import copy
import pyccl as ccl
# Warning: the current code is based on pyccl 2.x
# If you have 3.x installed, you need to change the pk2d function inputs
...
class bnt_core:
    ...
    def __init__(self,nz_data,ell_arr=None,**kwargs):
        ...
        self.aarr = np.flip(1./(1.+self.nz_dt[:,0]))
    ...
    def _inflate_core(self,**kwargs):
        ep = {"camb": {"halofit_version": self.nl_recipe, 
                       ...
                       "HMCode_A_des": 0.322, 
                       "HMCode_ST_des": 0.2}}
        if self.transfer == 'eisenstein_hu' or self.nl_code == 'emu':
    ...
    def kernel_clean(self,cosmo,z,nz):
        self.chi,kernel = ccl.tracers.get_lensing_kernel(cosmo,(z,nz),n_chi=len(z))
        return kernel
    ...
    def Cl_tomo(self):
        if self.IA_on == False:
            ...
            tracers = [ccl.tracers.WeakLensingTracer(cosmo=self.ccl_cosmo,dndz=(self.nz_dt[:,0],self.nz_dt[:,it+1]),ia_bias=(self.nz_dt[:,0],self.aia),use_A_ia=False) for it in range(self.ntomo)]
    ...
    def Cl_tomo2(self):
        ...
        for i in range(self.ntomo):
            ...
            if self.IA_on == False:
                ...
                ta = (1./(1+(self.nz_dt[:,0])[::-1]), self.aia[::-1])
    ...
    def aCl_tomo(self):
        ...
        for i in range(self.ntomo):
            ...
            if self.IA_on == False:
                ...
                # Warning, this is not the 100% precise way to implement IA
                # The correct method will be investigated in the following work
            bnt_tracer.append(Tracer_bnt)
        if self.mbias_ == None:
            aCls = np.array([[ccl.angular_cl(self.ccl_cosmo,bnt_tracer[i],bnt_tracer[j],self.ell_arr,p_of_k_a = self.pk2d) for i in range(self.ntomo)] for j in range(self.ntomo)])
        else:
            aCls = np.array([[(1+self.mbias[i])*(1+self.mbias[j])*ccl.angular_cl(self.ccl_cosmo,bnt_tracer[i],bnt_tracer[j],self.ell_arr,p_of_k_a = self.pk2d) for i in range(self.ntomo)] for j in range(self.ntomo)])
        self.aCls = aCls
